...
# Usage:
#
#   source llama.bash && LLM_MODEL_DIR=... LLM_MODEL_TUNING=1|2|3 llama_run_model
...
#
# Model: Qwen3.6-27B (8-bit quant, 128k context window, MTP)
#
llama_run_model() {
    ...
    # Regular model
    local -a ARGS=(--model "$LLM_MODEL_DIR/Qwen3.6-27B-UD-Q8_K_XL.gguf")
    ...
    # Want 16 checkpoints but runs out of memory with 64 GB
    ARGS+=(--cache-ram 2048 --ctx-checkpoints 1 --batch-size 1024 --ubatch-size 1024)

    # Multi Token Prediction (MTP), speeds up token generation.
    ARGS+=(--spec-type draft-mtp --spec-draft-n-max 2)

    # Model tuning: https://huggingface.co/unsloth/Qwen3.6-27B-MTP-GGUF
    case "${LLM_MODEL_TUNING:=0}" in
	1)
	    echo "Thinking mode for general tasks"
	    ARGS+=(--temp 1.0 --top-p 0.95 --top-k 20 --min-p 0.0 --presence-penalty 0.0 --repeat-penalty 1.0 --reasoning on)
	    ;;
	2)
	    ...
	    ;;
	3)
	    echo "Instruct (or non-thinking) mode"
	    ARGS+=(--temp 0.7 --top-p 0.80 --top-k 20 --min-p 0.0 --presence-penalty 1.5 --repeat-penalty 1.0 --reasoning off)
	    ;;
    esac
